...
import { deletePage, editPage, errorPage, guidePage, homePage, pastePage, } from './templates';
const MIMES = {
    'js': 'text/javascript',
    'css': 'text/css',
...
const XSS_OPTIONS = {
    whiteList: {
        ...
        ol: [],
        p: [],
        pre: ['class'], // Allow classes for syntax highlighting
        s: [],
        section: [],
        small: [],
        span: ['class'], // Allow classes for syntax highlighting
        sub: [],
        sup: [],
        strong: [],
...
function createParser() {
    ...
    const parse = (markdown, { toc = true } = {}) => {
        // Clear TOC items for each parse
        tocItems.length = 0;
...
function buildNestedList(items = [], level) {
    let html = '<ul>';
...
`;
        ...
        app.post('/save', async (req) => {
            ...
            let editCode = form.get('editcode');
